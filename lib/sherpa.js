...
Sherpa = {
  ...
  Response: function(path, params, parts) {
    this.path = path
  ...
  find: function(parts, request, params) {
    if (this.requestNode || this.destination && this.destination.route.partial) {
      ...
      if (target && target.destination && target.destination.route.partial) {
        return new Sherpa.Response(target.destination, params, parts);
      }
    }
    if (parts.length == 0) {
      var target = this;
      ...
      return target && target.destination ? new Sherpa.Response(target.destination, params, parts) : undefined;
    } else {
...
Sherpa.Router.prototype = {
  ...
  recognize: function(path, request) {
    ...
    var match = this.root.find(parts, request, []);
    if (match && match.route.partial && match.destination.recognize) {
      return match.destination.recognize("/"+match.parts.join('/'), request)
    } else {
      return match;
    }
  }
...
Sherpa.Route.prototype = {
  ...
  to: function(destination) {
    ...
    // provide the destination with the generated version of this route
    var self = this;
  
    if (this.destination) {
      this.destination.parentRoute = this;
    }
    return this;
  },
  ...
  generate: function(params) {
    ...
    if (path) {
      if (this.router.parentRoute) path = this.router.parentRoute.generate(params) + path;
      path = encodeURI(path);
      var query = '';
